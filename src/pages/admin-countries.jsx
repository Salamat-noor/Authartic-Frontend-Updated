...
import { useCreateCountryMutation, useFindAllCountriesQuery, useUpdateCountryMutation, useDeleteCountryMutation } from '@/slices/countriesApiSlice';
import { Box, Typography, TextField, IconButton, Menu, MenuItem, Dialog, DialogTitle, DialogContent, DialogActions, Button, Checkbox, Switch } from '@mui/material';
...
export default function PaginatedTable() {
    ...
    const [createCountry, { isLoading: isCreateCountryLoading, error: createCountryError }] = useCreateCountryMutation();
    const [updateCountry, { isLoading: isUpdateCountryLoading, error: updateCountryError }] = useUpdateCountryMutation();
    const [deleteCountry, { isLoading: isDeleteCountryLoading, error: deleteCountryError }] = useDeleteCountryMutation();
    ...
    useEffect(() => {
        if (data) {
            setColumns([
                ...
                { field: 'code', headerName: 'Code', minWidth: 150, headerClassName: 'bg-[#22477F] text-slate-100 text-md md:text-lg' },
                { field: 'name', headerName: 'Country', flex: 1, minWidth: 100, headerClassName: 'bg-[#22477F] text-slate-100 font-bold text-md md:text-lg' },
                {
                    field: 'update',
            ...
            setTableRows(data.data.length > 0 ? data.data : [{ id: 1, name: 'No countries found' }]);
            setDataLength(data.totalCount);
        }
        if (error) {
            setColumns([
                { field: 'status', headerName: 'Status Code', minWidth: 200, headerClassName: 'bg-[#22477F] text-slate-100 text-md md:text-lg' },
                { field: 'message', headerName: 'Message', flex: 1, minWidth: 250, headerClassName: 'bg-[#22477F] text-slate-100 text-md md:text-lg' },
            ]);
    ...
    return (
        <div className='w-screen min-h-screen flex flex-col justify-between bg-gray-100'>
            ...
            <Dialog open={openModal} onClose={() => setOpenModal(false)} fullWidth maxWidth="sm">
                <DialogTitle className="bg-blue-500 text-white">Edit Country</DialogTitle>
                <DialogContent className="bg-gray-100">
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, padding: "1rem" }}>
                        ...
                        <TextField
                            ...
                            className="bg-white"
                        />
                        <Box sx={{ display: 'flex', alignItems: 'center' }} className="bg-white p-2 rounded-md">
                            <Typography variant='body1' className="mr-2">Is Deleted</Typography>
                            <Checkbox
                                name="is_deleted"
            ...
            <Dialog open={openCreateModal} onClose={() => setOpenCreateModal(false)} fullWidth maxWidth="sm">
                <DialogTitle className="bg-blue-500 text-white">Create Country</DialogTitle>
                <DialogContent className="bg-gray-100">
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, padding: "1rem" }}>
                        <TextField
                            label="Name"
                ...
                <DialogActions className="bg-gray-200">
                    ...
                    <Button onClick={handleCreateSubmit} variant="contained" color="primary">Create Country</Button>
                </DialogActions>
            </Dialog>
        </div>
    );
